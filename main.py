...
def make_neopixel_clock_gui():
    
    guizero_app = guizero.App(title="Neopixel tester")
    
    pin = board.D18 #GPIO 18 = rpi pin 12
    led_count = 35
    brightness_max = 10
    clock = NeopixelRingClock(pin, led_count, brightness_max)
    ...
    slider_hour = guizero.Slider(box_hour, command=handle_time_slider, start=0, end=11, width="fill", align="left") 
...
def make_end_user_gui():
    ...
    def update_daily_alarm():
        ...
        is_pm = now.hour >= 12
#        date_to_get_sunrise_for = None #apparently we don't need to forward-declare this
